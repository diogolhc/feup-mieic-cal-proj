...
De forma a controlar a distribuição de vacinas contra a COVID-19, a aplicação VaccineRouter deve ser capaz de determinar os itinerários de distribuição de vacinas desde os centros de armazenamento até aos centros de aplicação. Como tal, pretende-se que estes itinerários não sejam demasiado extensos, de forma a prevenir que o tempo de distribuição ultrapasse o tempo de conservação das vacinas.
	...
	Na terceira fase será adicionado o fator de tempo limite de distribuição. Tal como descrito anteriormente, as vacinas têm um tempo limitado de transporte, isto é, 
	não podem estar muito tempo nas carrinhas que as irão distribuir (em transporte). \par
...
fortemente conexas, sendo que para a visualização dos mesmos é utilizada 
...
Desta forma, é possível comparar os resultados obtidos pelos dois algoritmos, nomeadamente o número de vértices que cada um processa. \par
Analisando as imagens seguintes, comprova-se que o algoritmo \hyperref[algo:astar]{A*} processa menos vértices do que o algoritmo de \hyperref[algo:dijkstra]{Dijkstra} para obter a solução. \par
...
É pedido como input o tempo limite de conservação, em minutos, das vacinas. Neste programa é assumido que a velocidade das carrinhas
...
sendo posteriormente mostrado o tempo dispendido por cada carrinha. Para além disto, é mostrado o grafo utilizado
...
Se numa aresta passar mais do que uma carrinha ou passar a mesma carrinha mais do que uma vez, essa aresta é colorida a vermelho para dar essa indicação. \par

O programa recebe ainda um outro input que permite ao utilizador escolher se deseja visualizar os caminhos mais rápidos possíveis mesmo que o tempo limite seja ultrapassado.
...
para a representação das mesmas. \par
...
Para o cálculo do \hyperref[perspetivas:caminhomaiscurto]{Caminho mais curto}, foram implementados os algoritmos \hyperref[algo:dijkstra]{Dijkstra} e \hyperref[algo:astar]{A*}. 
Uma vez que estes algoritmos são gananciosos, foram feitas alterações nos mesmos de modo a que, quando se atinge o ponto de
interesse mais próximo, estes terminem, evitando a análise de vértices sem interesse e, portanto, melhorando a sua eficiência.
...
De forma a resolver o \hyperref[perspetivas:tsp]{Travelling Salesman Problem} foi implementado o algoritmo \hyperref[algo:nn]{Nearest Neighbor}, descartando-se a implementação
do algoritmo \hyperref[algo:bellmanheldkarp]{Bellman-Held-Karp}, uma vez que apresenta uma complexidade temporal muito elevada de \( O(|V|^2 \cdot 2^{|V|})\).
...
Por outro lado, uma vez que pode haver vários centros de armazenamento, foi necessário resolver o \hyperref[perpetivas:mdvrp]{Multi-depot Vehicle Routing Problem}. 
...
é de ordem constante \(O(|V|)\), uma vez que a implementação deste algoritmo necessita de guardar os vértices ainda não visitados numa \textit{queue}.
Para se fazer a análise empírica do algoritmo, foi utilizado o algoritmo quinze vezes em cada mapa (Porto, PortoSCC, Espinho, EspinhoSCC, Penafiel, PenafielSCC).
O gráfico representado de seguida mostra os resultados obtidos em ordem à complexidade temporal teórica:
...
De forma a resolver o problema proposto, foram conjugados os algoritmos descritos acima: de modo a verificar se existe caminho de ida e de volta entre
os centros de distribuição e aplicação, aplicou-se o algoritmo \hyperref[algo:kosaraju]{Kosaraju}, obtendo-se as componentes fortemente conexas; De seguida, fez-se 
...
utilizou-se o algoritmo \hyperref[algo:nn]{Nearest Neighbor}. Uma vez que o objetivo do problema é distribuir as vacinas num dado tempo limite, foi feita uma divisão 
recursiva dos centros de aplicação pertencentes ao cluster de um dado centro de distribuição. Deste modo, conseguiu-se minimizar o número de carrinhas utilizadas, cumprindo o tempo 
de conservação das vacinas. Esta divisão foi feita a partir da coordenada cuja amplitude é maior do conjunto atual de centros de aplicação, aplicando-se aqui uma abordagem gananciosa. 
Esta abordagem implica, assim, um aumento da complexidade num fator de $\log(|POI|)$.
Uma vez que esta implementação final é uma junção destes vários algoritmos, acrescido do facto de se fazer a divisão descrita anteriormente, a ordem da sua complexidade temporal será igual à ordem da complexidade temporal
do algoritmo com maior complexidade temporal multiplicada pela complexidade da divisão. Assim, a complexidade temporal da implementação será da ordem de \( O(\log|POI| \cdot |POI| \cdot (|V| + |E|)\cdot \log|V|) \).
...
De acordo com a informação dada no problema, pode haver obras na via pública que podem tornar algumas zonas inacessíveis.
Mais do que isto, torna-se ainda necessário que, não só as carrinhas de distribuição consigam chegar aos centros de
aplicação aos quais se pretendem levar as vacinas, mas também que tenham caminho de volta. Para este fim, recorreu-se
ao algoritmo de \hyperref[algo:kosaraju]{Kosaraju}, que divide o grafo dado nas suas componentes fortemente conexas.
...
posteriormente, uma média pesada dos resultados obtidos em situações diferentes. Recolhidos estes resultados, tornou-se possível confirmar as complexidades
temporais teóricas dos algoritmos implementados.

Para a realização da solução face ao problema descrito, fez-se, inicialmente, o estudo dos algoritmos dados nas aulas da unidade curricular de Concepção e Análise de Algoritmos. 
Posteriormente, foi feito um estudo do problema em questão e de possíveis algoritmos para a resolução do mesmo.
...
algoritmos face aos problemas onde serão aplicados. Desta forma, este projeto revelou-se importante para o nosso desenvolvimento individual e 
