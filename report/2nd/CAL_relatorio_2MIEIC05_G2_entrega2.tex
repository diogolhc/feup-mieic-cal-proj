...
Para efetuar a análise da Acessibilidade, foram implementados os algoritmos \hyperref[algo:dfs]{DFS} e \hyperref[algo:bfs]{BFS}. \par

Para a análise da complexidade da Conectividade forte do grafo, foi implementado o algoritmo \hyperref[algo:kosaraju]{Kosaraju} uma vez que 
...
Para o cálculo do Caminho mais curto, foram implementados os algoritmos \hyperref[algo:dijkstra]{Dijkstra} e \hyperref[algo:astar]{Astar}. 
Uma vez que estes algoritmos são algoritmos gananciosos e garantem a solução ótima com os grafos utilizados, foram feitas alterações nos mesmos de modo a
que, quando se atinge o ponto de interesse mais próximo, estes terminam, de forma a evitar a análise de vértices sem interesse e, portanto, melhorar a sua eficiência.
...
uma complexidade muito elevada de \( O(|V|^3*2^(|V|)) \), pelo que foi implementado o algoritmo \hyperref[algo:nn]{Nearest neighbor}.
...
mais próximo, é escolhido o ponto de interesse mais próximo. \par

Uma vez que podem haver vários centros de distribuição, foi necessário resolver o \hyperref[perspetivas:mdvrp]{Multi-depot Vehicle Routing Problem}. Tal como 
descrito na seccão \hyperref[perspetivas:mdvrp]{3.6}, para resolver o problema foi implementado o algoritmo \hyperref[algo:msdijkstra]{Multi-source Dijkstra}. \par 
