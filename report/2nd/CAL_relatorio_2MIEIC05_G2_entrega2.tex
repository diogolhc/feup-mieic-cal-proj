...
Mais do que isto, torna-se ainda necessário que não só uma carrinha de deistribuição consegue chegar aos centro de
aplicação aos quais se pretende levar as vacinas, mas também que tenha caminho de volta. Para este fim recorreu-se
ao algoritmo de \hyperref[algo:kosaraju]{Kosaraju} que dividiu o grafo dado em componentes fortemente
conexas de modo a fazer a distribuição de vacinas de centros de armazenamento para centros de aplicação que pertencem
à mesma componente fortemente conexa de modo a assegurar que existe um caminho de volta. \par

Utilizando a interface do nosso programa é possível visualizar de forma distinta os vertices pertencentes à
componente fortemente conexa maior das restantes componentes conexas, sendo que estas últimas são em grande número
e cada uma com reduzido número de vértices. \par
Nas imagens que se seguem, como já mencionado em \hyperref[func]{Funcionalidades e cenários implementados}, 
a componente maior é mostrada a amarelo e as restantes a azul.
