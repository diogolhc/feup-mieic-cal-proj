...
\chapter{Formalização do problema}
	...
	\begin{itemize}
		...
		\begin{itemize}
			\item \( V_i \) - Vértices, onde \( V_i(i) \) representa o i-ésimo vértice, representando os centros de distribuição e de aplicação, com:
			\begin{itemize}
	...
	Assim, primeiro pretende-se diminuir o tempo despendido pela carrinha que demora mais tempo, de forma que as vacinas não estejam em transporte mais do que o tempo limite, e, secundariamente, o número de carrinhas que irão transportar as vacinas todas pretendidas aos centros de administração.
...
O facto de termos vários centros de distribuição leva a que este problema vá ao encontro da generalização do \textit{Vehicle routing problem}, ou seja, entramos no âmbito de \textit{Multi-depot vehicle routing problem}.
	...
	\end{itemize}
		...
		Uma possível solução para este problema é a utilização do algoritmo de \textit{Bellman-Held-Karp}. \par
		...
		Este algoritmo oferece-nos uma complexidade temporal relativamente boa (apesar de ainda exponencial) de \( O(|V|^2 \cdot 2^{|V|})\) mas, por outro lado, uma complexidade espacial \( O(|V| \cdot 2^{|V|})\), ambas no pior caso. \cite{bellmanheldkarp}
		...
		\begin{center}
			...
			\begin{algorithmic}[1]
				\State $G = (V, E)$
				\newline
				\Function{BellmanHeldKarp}{$G,c$}
				\State $s \leftarrow inicial$ $vertex$
				\For{$v \in V$}
					\State $OPT[\{v\}, v] \leftarrow c(s,v)$
				\EndFor
				\For{$i=2$ to $|V|$}
					\For{$S \subseteq V-s$ with $|S| = i$}
						\For{$v \in S$}
							\State $ OPT[S,v] \leftarrow min\{OPT[S - v, u] + c(u, v)$ | $u \in S - v \})$
						\EndFor
					\EndFor
				\EndFor
				\State $return$ $min\{ OPT[V - s, v] + c(v, s)$ | $v \in V - s \} $
				\EndFunction
			\end{algorithmic}
		\end{center}
		...
		No caso em concreto deste trabalho, em vez de escolher o vértice adjacente mais próximo, é escolhido o ponto de interesse mais próximo, que pode ser, por exemplo, obtido usando o algoritmo \textit{A*} com origem no vértice atual. \par
		O resultado é a ordem pela qual os vértices foram visitados. \par
		Este sendo um algoritmo que procura obter a solução a partir da melhor escolha em cada momento (indo ao vértice mais próximo), revela-se um algoritmo ganancioso e, no pior caso, tem uma complexidade temporal de \(O(|V|^2) \) e espacial de \( O(|V|)\).
